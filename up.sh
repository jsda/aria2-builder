getversion(){
curl -H "Authorization: Bearer $GITHUB_TOKEN" -sL https://api.github.com/repos/$1/releases/latest | grep -o '"tag_name": ".*"' | head -n 1 | sed 's/"//g;s/v//g' | sed 's/tag_name: //g' | sed 's/release-//g'
}
...
OPENSSL=$(curl -sL https://openssl-library.org/source/ | grep -A2 '\[LTS\]' | grep -oE 'openssl-[0-9]+\.[0-9]+\.[0-9]+\.tar\.gz' | head -n1 | awk '{print "https://github.com/openssl/openssl/releases/download/" $0 "/" $0}')
SQLITE3='https://sqlite.org/$(curl -sL https://sqlite.org/download.html | grep -o '..../sqlite-autoconf-.*.tar.gz' | head -n 1)'
