...
openssl=$(curl -s https://openssl-library.org/source/ | grep -A2 '\[LTS\]' | grep -oE 'openssl-[0-9]+\.[0-9]+\.[0-9]+\.tar\.gz' | head -n1 | awk '{print "https://github.com/openssl/openssl/releases/download/" $0 "/" $0}')
