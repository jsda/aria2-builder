getversion(){
curl -H "Authorization: Bearer $GITHUB_TOKEN" -s https://api.github.com/repos/$1/releases/latest | grep -o '"tag_name": ".*"' | head -n 1 | sed 's/"//g;s/v//g' | sed 's/tag_name: //g' | sed 's/release-//g'
}
